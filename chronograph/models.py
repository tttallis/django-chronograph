...
def schedule_single_job(eta=None, command='', argstring='', name=''):
    now = datetime.now()
    name = name or command
    if not eta:
        return Job.objects.get_or_create(name=name, command=command, args=argstring, next_run=now)
    elif eta >= now:
        return Job.objects.get_or_create(name=name, command=command, args=argstring, next_run=eta)
    return False